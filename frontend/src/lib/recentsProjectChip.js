...
//
// The chats-list backend now emits `row.project = { id, name } | null` (see the
...
// Returns { id, name } for the chip, or null when the row shows none. Only chat
// rows carry a project; an app row never does. A malformed/partial project
...
export function recentsProjectChip(kind, item) {
  ...
  const name = typeof project.name === 'string' && project.name.trim()
    ? project.name.trim()
    : 'Project'
  return { id: String(project.id), name }
}
