...
SOCKET_PATH = Path(
  os.environ.get(
    ...
    str(DATA_DIR / "run" / "mobius-identity-broker.sock"),
  )
)
TCP_HOST = "127.0.0.1"
...
GATEWAY_BASE_URL = os.environ.get(
  "MOBIUS_AGENT_GATEWAY_URL", "https://compute.mobius.you"
).rstrip("/")
...
class Broker:
  ...
  def retry_pending_loop(self, stop: threading.Event) -> None:
    delay = 1.0
    while not stop.is_set() and PENDING_BOOTSTRAP_PATH.exists() and not self.state:
    ...
    *,
    ...
    if route is None and allow_contributions:
      if method == "POST" and route_path == "/v1/contributions" and not split.query:
        ...
        method == "GET"
        and route_path == "/v1/contributions/github/status"
        and not split.query
      ):
        route = (
        ...
        method == "DELETE"
        and route_path == "/v1/contributions/github"
        and not split.query
      ):
        route = (
          "contribution:submit", CONTRIBUTION_BASE_URL,
