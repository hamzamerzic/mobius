...
A project artifact is a buildable output (a rendered website or a compiled
LaTeX PDF) whose source lives in the project tree and whose build status is
persisted on ``Project.artifacts_json``. This module owns the two builders,
the in-memory registry of live build tasks, and ``run_build`` — the common
orchestrator that transitions status, runs the builder, and broadcasts the
result.
...
def default_output_rel(artifact_id: str, builder: str, source: str) -> str:
...
def new_artifact_entry(
  ...
  return {
    ...
    "builder": builder,
    "source": source,
    "output_rel": default_output_rel(artifact_id, builder, source),
    "status": "idle",
    "updated_at": _now_iso(),
    "duration_ms": None,
...
def _build_context(
  ...
  """Resolve the project root, builder name, and source for a build."""
  from app import models
  ...
  return root, str(builder), str(source) if isinstance(source, str) else ""
...
async def run_build(project_id: str, artifact_id: str) -> None:
  ...
  async with project_build_lock(project_id):
    ...
    if context is None:
      ...
      return
    root, builder_name, source = context
    ...
    try:
      ...
      if builder is None:
        _write_log(log_path, f"Unknown builder: {builder_name}\n")
      elif not source or source_path is None or not source_path.is_file():
