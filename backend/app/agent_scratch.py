...
def sweep_idle_scratch(db: Session, *, now: float | None = None) -> dict:
  ...
  Returns a summary rather than logging alone so a caller — today the runner,
  later a retention loop — can report what it reclaimed. Failure to remove one
  directory must not prevent the rest, so errors are collected, not raised: a
  sweep is opportunistic and its caller is mid-turn.
  """
